...
- `vercel.json` `functions.api/index.ts.includeFiles` (`content/**`, `server/db/schema.sql`) is **required** — `@vercel/node` bundles the api function with `@vercel/nft`, which cannot statically trace `registry.ts`'s dynamic `readdirSync`/`readFileSync(join(...))` or `migrate.ts`'s `schema.sql` read. Without it those files are missing from the bundle and cold start throws `ENOENT` (`FUNCTION_INVOCATION_FAILED`). If you add server-side content files outside `content/`, extend this list; do not remove it.
