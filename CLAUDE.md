...
- **ESM relative imports must use explicit `.js` extensions** (e.g. `import { fmtInt } from "./format.js"`). Vercel runs the api function as native ESM; an extensionless import (`./format`) fails at cold start with `ERR_MODULE_NOT_FOUND` (`FUNCTION_INVOCATION_FAILED`) even though tsx/vitest resolve it fine. This caused a full production outage (fixed by adding `.js`); keep new imports explicit.
- Do **not** add a `functions` config to vercel.json — in this project's custom `buildCommand` + `outputDirectory` setup, `functions.includeFiles` fails the Vercel build (function pattern doesn't match). It was also unnecessary: `content/` + `schema.sql` are already traced into the api function bundle by the `@vercel/nft` version Vercel ships (v1.10), despite the dynamic `readdirSync`/`readFileSync(join(...))`. If content ever stops bundling, prefer static JSON imports (nft traces those) over `functions.includeFiles`.
